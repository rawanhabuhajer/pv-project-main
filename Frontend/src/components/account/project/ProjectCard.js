...
import { deletePvProjects, editPvProjects } from "@/store/actions";
  ...
  return (
    <div className="project-card">
      <div className="card-header">
        ...
        <div className="card-date-wr">
          <div className="deadline">
            <CalendarDays size={16} /> Deadline :{" "}
            {getFullDate(project?.deadline)}
          </div>

          <div className="badge">
            ( {project?.subcategories?.length || 0}{" "}
            {project?.subcategories?.length === 1
              ? "sub project"
              : "sub projects"}{" "}
            )
          </div>
          ...
          <Circle
            percent={
              project?.subcategories?.length
                ? (project.subcategoryStatus3Count /
                    project.subcategories.length) *
                  100
                : 0
            }
